...
client.on('ready', async () => {
  ...
  const originalPermissions = await client.application?.commands.permissions.fetch({
    guild: testGuildId,
  });
  ...
  await globalPermissionsManager?.set({
    guild: testGuildId,
    fullPermissions: [
      { id: globalCommandId, permissions: [{ type: 'ROLE', id: testGuildId, permission: true }] },
    ],
  });
...
client.on('messageCreate', async (message) => {
  ...
  if (message.inGuild()) {
    ...
    expectAssignable<(test: ButtonInteraction<'cached'>) => boolean | Promise<boolean>>(
  ...
  expectAssignable<Promise<SelectMenuInteraction>>(
    message.awaitMessageComponent({ componentType: 'STRING_SELECT' })
  );
  expectAssignable<Promise<SelectMenuInteraction>>(
    channel.awaitMessageComponent({ componentType: 'STRING_SELECT' })
  );
...
declare const applicationCommandManager: ApplicationCommandManager;
{
  ...
  expectAssignable<Promise<ApplicationCommand>>(
...
declare const applicationSubGroupCommandData: ApplicationCommandSubGroupData;
{
  ...
  expectType<ApplicationCommandSubCommandData[] | undefined>(
...
declare const categoryChannel: CategoryChannel;
{
  ...
  expectType<Promise<StageChannel>>(
    categoryChannel.createChannel('name', { type: 'GUILD_STAGE_VOICE' })
  );
...
declare const guildChannelManager: GuildChannelManager;
{
  ...
  expectType<Promise<CategoryChannel>>(
  ...
  expectType<Promise<Collection<Snowflake, AnyChannel | null>>>(
...
declare const guildBanManager: GuildBanManager;
{
  ...
  expectType<Promise<GuildBan>>(
    guildBanManager.fetch({ user: '1234567890', cache: true, force: false })
  );
  expectType<Promise<Collection<Snowflake, GuildBan>>>(guildBanManager.fetch());
  expectType<Promise<Collection<Snowflake, GuildBan>>>(guildBanManager.fetch({}));
  expectType<Promise<Collection<Snowflake, GuildBan>>>(
    guildBanManager.fetch({ limit: 100, before: '1234567890' })
  );
  // @ts-expect-error
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.isMessageComponent()) {
    ...
    expectType<MessageActionRowComponent | APIButtonComponent | APISelectMenuComponent>(
  ...
  if (interaction.isCommand()) {
    if (interaction.inRawGuild()) {
      ...
      expectType<Promise<APIMessage>>(interaction.reply({ fetchReply: true }));
      expectType<APIInteractionDataResolvedGuildMember | null>(
        interaction.options.getMember('test')
      );
      expectType<APIInteractionDataResolvedGuildMember>(
        interaction.options.getMember('test', true)
      );
      ...
      expectType<APIInteractionDataResolvedGuildMember | GuildMember>(
        interaction.options.getMember('test', true)
      );

      expectType<GuildBasedChannel | APIInteractionDataResolvedChannel>(
    ...
    expectType<CommandInteraction>(interaction);
    expectType<Omit<CommandInteractionOptionResolver<CacheType>, 'getFocused' | 'getMessage'>>(
      interaction.options
    );
...
expectType<Promise<GuildAuditLogs<AuditLogEvent.MemberKick>>>(
  guild.fetchAuditLogs({ type: AuditLogEvent.MemberKick })
);

expectType<Promise<GuildAuditLogs<'CHANNEL_CREATE'>>>(
...
expectType<Promise<GuildAuditLogs<'INTEGRATION_UPDATE'>>>(
  guild.fetchAuditLogs({ type: 'INTEGRATION_UPDATE' })
);
...
expectType<Promise<GuildAuditLogs<null>>>(
  guild.fetchAuditLogs({ type: GuildAuditLogs.Actions.ALL })
);
expectType<Promise<GuildAuditLogs<'ALL'>>>(guild.fetchAuditLogs());

expectType<
  Promise<GuildAuditLogsEntry<'MEMBER_KICK', 'MEMBER_KICK', 'DELETE', 'USER'> | undefined>
>(guild.fetchAuditLogs({ type: 'MEMBER_KICK' }).then((al) => al.entries.first()));
expectType<
  Promise<GuildAuditLogsEntry<'MEMBER_KICK', 'MEMBER_KICK', 'DELETE', 'USER'> | undefined>
>(
  guild
    .fetchAuditLogs({ type: GuildAuditLogs.Actions.MEMBER_KICK })
...
expectType<DMChannel | PartialDMChannel | NewsChannel | TextChannel | ThreadChannel | VoiceChannel>(
  TextBasedChannel
);
expectType<
  | 'DM'
