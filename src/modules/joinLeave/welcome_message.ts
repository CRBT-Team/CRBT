...
export default ChatCommand({
  ...
  async handle() {
    ...
    await this.reply(await render(data?.joinMessage, this.locale));
  },
});
        ...
        new MessageEmbed()
          .setAuthor({
            name: t(this.locale, 'JOINLEAVE_MESSAGE_SAVE_TITLE').replace(
              '<TYPE>',
              t(this.locale, 'JOIN_MESSAGE')
            ),
            iconURL: icons.success,
          })
          .setDescription(t(this.locale, 'JOIN_MESSAGE_SAVE_DESCRIPTION'))
